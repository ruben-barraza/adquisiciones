...
class Pogeneralmodel extends CI_Model
{
    ...
    function get_idImc($idPog, $idArticulo, $idProveedor, $partida, $plazoEntrega){
        $this->db->select('id');
        ...
        $this->db->where('plazoEntrega', $plazoEntrega);
        $query = $this->db->get();
        if($query->num_rows() > 0){
            $vl = $query->row_array();
            return $vl['id'];
    ...
    function get_idImcH($idPog, $idArticulo, $idProveedor, $partida){
        $this->db->select('id');
        $this->db->from('im_concepto');
        $this->db->where('idPog', $idPog);
        $this->db->where('idArticulo', $idArticulo);
